...
$(function() {
  var boton, doSearch, form, input, resultados, searchReq, searchTO, total;
  ...
  doSearch = function() {
    ...
    searchReq.done(function(data) {
      var html, resultado, _i, _len, _ref;
      total.text(data.total);
      html = '';
      _ref = data.resultados;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        resultado = _ref[_i];
        html += Templates['representante'](resultado);
      }
      return $('#resultados').html(html);
    });
    return searchReq.always(function() {
      return boton.removeAttr('disabled');
    });
  };
  input.on('keyup', function(evt) {
    clearTimeout(searchTO);
    return searchTO = setTimeout(doSearch, 200);
  });
  return $('#buscador').on('submit', function(evt) {
    evt.preventDefault();
    return doSearch();
  });
});
