...
$(function() {
  var MuestraRepresentantes, MuestraSeccion, MuestraUnRepresentante, mapita, mostrandoRep, navigation, newPosition, opts, quitaDetalles;
  ...
  mapita.on('map.loaded', function() {
    var centro;
    $('#mapa').removeClass('loading').on('click', function(evt) {
      evt.preventDefault();
      return $('#instrucciones-mapa').fadeOut();
    });
    ...
    return $('#mapa').on("webkitAnimationEnd animationend", function(evt) {
      return $('#mapa-loading').css('display', 'none');
    });
  });
  navigation = {};
  ...
  quitaDetalles = function(evt) {
    ...
    return $('body').removeClass('covered');
  };
  MuestraUnRepresentante = function(actor) {
    var grafica_asistencias, grafica_votaciones, mostrandoRep;
    if (mostrandoRep) {
      return;
    }
    ...
    NotificationCenter.on('resize', function() {
      ...
      return grafica_votaciones.resize();
    });
    $('#cover .hoja').click(function(evt) {
      return evt.stopPropagation();
    });
    return mostrandoRep = false;
  };
  MuestraRepresentantes = function(reps) {
    var activos, actor, filtros, html, id, inactivos;
    html = '';
    for (id in reps) {
      actor = reps[id];
      html += actor;
  ...
  MuestraSeccion = function(seccion) {
    return mapita.setPolygons(seccion.coords.coordinates);
  };
  navigation['representantes-lista'] = function(data) {
    var actores;
    quitaDetalles();
    actores = Representantes.parse(data.representantes);
    console.log(actores);
    MuestraRepresentantes(actores);
    return MuestraSeccion(data.seccion);
  };
  navigation['representantes-show'] = function(data) {
    var actor;
    if (mostrandoRep) {
      return;
    }
    ...
    return MuestraUnRepresentante(actor);
  };
  History.Adapter.bind(window, 'statechange', function() {
    ...
    if (State.hash === '/') {
      ...
      return navigation[State.data.evt] && navigation[State.data.evt](State.data.data);
    }
  });
  NotificationCenter.on('history.change', function(data) {
    var url;
    return url = data.url;
  });
  if (window.request_data) {
  ...
  newPosition = function(evt) {
    ...
    return Representantes.deUbicacion(coords, function(representantes, seccion) {
      ...
      if (representantes === 'error') {
        ...
        setTimeout(function() {
          return $('#instrucciones-mapa').fadeOut(500);
        }, 3000);
        return false;
      }
  ...
  Geo.on('ubicando', function(evt) {
    return $('#localizame .icono').text('loading').addClass('loading');
  });
  Geo.on('posicion', function(evt) {
    newPosition(evt);
    return $('#localizame .icono').text('locate').removeClass('loading');
  });
  $('#aqui-vivo').on('click', newPosition);
  $('#localizame').on('click', function(evt) {
    $('#instrucciones-mapa').fadeOut();
    evt.preventDefault();
    return Geo.setup();
  });
  $('#cover').appendTo('body');
  mostrandoRep = false;
  $('#representantes').on('click', '.representante', function(evt) {
    var actor, id;
    evt.preventDefault();
    id = this.id.split('-')[1];
